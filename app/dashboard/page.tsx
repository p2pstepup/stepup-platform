  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (!user) {
        ...
        setUser(user)
        const [{ data: notifs }, { data: profileData }] = await Promise.all([
          supabase.from('notifications').select('*').eq('student_id', user.id).order('created_at', {ascending: false}).limit(20),
        ...
        <div style={{display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 28}}>
          <div>
            <div style={{fontFamily: 'Georgia, serif', fontSize: 30, color: '#0d2340', letterSpacing: -0.5}}>
              Good {greeting}, {profile?.full_name?.split(' ')[0] || user?.email?.split('@')[0]}
            </div>
            <div style={{fontSize: 14, color: '#8a7d6a', marginTop: 5}}>
              {new Date().toLocaleDateString('en-US', {weekday: 'long', month: 'long', day: 'numeric'})} · Week 1 of 8
            </div>
          </div>
            ...
            {showNotifications && (
              <div style={{position: 'absolute', top: 52, right: 0, width: 360, background: 'white', border: '0.5px solid #e8dfc8', borderRadius: 12, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', zIndex: 200}}>
                <div style={{padding: '14px 18px', borderBottom: '0.5px solid #f0ece0', display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
                  ...
                  {unreadCount > 0 && <div onClick={markAllRead} style={{fontSize: 12, color: '#c9a84c', cursor: 'pointer'}}>Mark all read</div>}
                </div>
