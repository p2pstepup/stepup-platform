...
export default function HYNotes() {
  ...
  const navGroups = [
    ...
    {section: 'Resources', items: [
      {name: 'HY Topic Notes', path: '/dashboard/notes', active: true},
  ...
  const filtered = notes.filter(n => n.topic.toLowerCase().includes(search.toLowerCase()) || n.category.toLowerCase().includes(search.toLowerCase()))
    ...
    <main style={{minHeight: '100vh', display: 'flex', background: '#f7f4ee', fontFamily: 'Sora, sans-serif', fontSize: '17.6px'}}>
      ...
      <div style={{flex: 1, minWidth: 0, overflowY: 'auto', padding: '32px 36px'}}>
        <div style={{marginBottom: 28}}>
          <div style={{fontFamily: 'Georgia, serif', fontSize: 30, color: '#0d2340', letterSpacing: -0.5}}>HY Topic Notes</div>
          <div style={{fontSize: 14, color: '#8a7d6a', marginTop: 5}}>High yield notes for every tested topic · Click to open</div>
        </div>

        <input type="text" value={search} onChange={e => setSearch(e.target.value)}
          placeholder="Search topics..."
          style={{width: '100%', height: 46, borderRadius: 10, border: '1px solid #e8dfc8', background: 'white', fontFamily: 'Sora, sans-serif', fontSize: 14, padding: '0 16px', color: '#1a1008', outline: 'none', boxSizing: 'border-box', marginBottom: 20}}/>
            ...
            <div key={cat} style={{background: 'white', border: '0.5px solid #e8dfc8', borderRadius: 12, overflow: 'hidden', marginBottom: 16}}>
              ...
              {catNotes.map((note, i) => (
                <div key={note.id}
                  ...
                  style={{display: 'flex', alignItems: 'center', gap: 14, padding: '14px 20px', borderBottom: i < catNotes.length-1 ? '0.5px solid #f5f0e8' : 'none', cursor: note.link ? 'pointer' : 'default'}}
                  onMouseEnter={e => note.link && (e.currentTarget.style.background = '#fafaf8')}
                  onMouseLeave={e => (e.currentTarget.style.background = 'white')}>
                  <div style={{width: 40, height: 40, background: '#f0f7f8', border: '1px solid #c9d8dc', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0}}>
                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                      <path d="M4 2h7l4 4v10a1 1 0 01-1 1H4a1 1 0 01-1-1V3a1 1 0 011-1z" stroke="#4a8c84" strokeWidth="1.2" strokeLinecap="round"/>
                      <path d="M11 2v4h4M6 9h6M6 12h4" stroke="#4a8c84" strokeWidth="1.2" strokeLinecap="round"/>
                    </svg>
                  </div>
                  <div style={{flex: 1}}>
                    <div style={{fontSize: 14, fontWeight: 500, color: '#0d2340', marginBottom: 3}}>{note.topic}</div>
                    {note.description && <div style={{fontSize: 12, color: '#8a7d6a'}}>{note.description}</div>}
                  </div>
                  {note.link ? (
                    <div style={{padding: '7px 16px', background: '#0d2340', borderRadius: 8, fontSize: 12, color: '#c9a84c', fontWeight: 500, flexShrink: 0}}>Open notes ↗</div>
                  ) : (
                    <div style={{fontSize: 12, color: '#a89870', fontStyle: 'italic', flexShrink: 0}}>Coming soon</div>
                  )}
                </div>
              ))}
            </div>
