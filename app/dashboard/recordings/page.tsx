...
export default function SessionRecordings() {
  ...
  const navGroups = [
    ...
    {section: 'Resources', items: [
      ...
      {name: 'Session Recordings', path: '/dashboard/recordings', active: true},
  ...
  const weeks = [...new Set(recordings.map(r => r.week_number))].sort()
    ...
    <main style={{minHeight: '100vh', display: 'flex', background: '#f7f4ee', fontFamily: 'Sora, sans-serif', fontSize: '17.6px'}}>
      ...
      <div style={{flex: 1, minWidth: 0, overflowY: 'auto', padding: '32px 36px'}}>
        <div style={{marginBottom: 28}}>
          <div style={{fontFamily: 'Georgia, serif', fontSize: 30, color: '#0d2340', letterSpacing: -0.5}}>Session Recordings</div>
          <div style={{fontSize: 14, color: '#8a7d6a', marginTop: 5}}>All session recordings are posted within 24 hours · You'll be notified when new recordings are available</div>
        </div>
        ...
        <div style={{background: '#0d2340', borderRadius: 12, padding: '16px 22px', marginBottom: 24, borderLeft: '4px solid #c9a84c'}}>
          <div style={{fontSize: 14, color: 'rgba(255,255,255,0.7)', lineHeight: 1.6}}>
            Recordings are posted here after each session. Watch them to review material or catch up on anything you missed. You'll receive a notification when new recordings are posted.
          </div>
        </div>

        {recordings.length === 0 ? (
          <div style={{background: 'white', border: '0.5px solid #e8dfc8', borderRadius: 12, padding: '60px', textAlign: 'center'}}>
            <div style={{fontSize: 16, fontWeight: 500, color: '#0d2340', marginBottom: 8}}>No recordings yet</div>
            <div style={{fontSize: 14, color: '#8a7d6a'}}>Session recordings will appear here after the program begins on May 4th.</div>
          </div>
        ) : (
          weeks.map((week) => (
            <div key={week} style={{background: 'white', border: '0.5px solid #e8dfc8', borderRadius: 12, overflow: 'hidden', marginBottom: 16}}>
              ...
              {recordings.filter(r => r.week_number === week).map((rec, i, arr) => (
                <div key={rec.id} style={{display: 'flex', alignItems: 'center', gap: 16, padding: '14px 20px', borderBottom: i < arr.length-1 ? '0.5px solid #f5f0e8' : 'none'}}>
                  <div style={{width: 44, height: 44, background: rec.available ? '#0d2340' : '#f7f4ee', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0}}>
                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                      <circle cx="9" cy="9" r="7" stroke={rec.available ? '#c9a84c' : '#a89870'} strokeWidth="1.2"/>
                      <path d="M7 6.5l5 2.5-5 2.5V6.5z" fill={rec.available ? '#c9a84c' : '#a89870'}/>
                    </svg>
                  </div>
                  <div style={{flex: 1}}>
                    <div style={{fontSize: 14, fontWeight: 500, color: '#0d2340', marginBottom: 3}}>{rec.topic}</div>
                    <div style={{fontSize: 12, color: '#8a7d6a'}}>
                      {new Date(rec.session_date).toLocaleDateString('en-US', {weekday: 'long', month: 'long', day: 'numeric'})}
                      {rec.duration && ` · ${rec.duration}`}
                    </div>
                  </div>
                  {rec.available && rec.link ? (
                    <a href={rec.link} target="_blank" rel="noopener noreferrer"
                      style={{padding: '8px 18px', background: '#0d2340', borderRadius: 8, fontSize: 13, color: '#c9a84c', fontWeight: 600, textDecoration: 'none'}}>
                      Watch ↗
                    </a>
                  ) : (
