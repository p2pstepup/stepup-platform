...
function rescoreSession(answers: Record<string, string>): {
  correctedScore: number
  questionResults: Array<{
    correctedQ: number
    originalQ: number | null
    action: string
  ...
  for (const entry of RESCORING_MAP) {
    ...
    if (entry.action === 'AWARD_AUTO_CREDIT') {
      ...
      // For both GRADE_NORMALLY and RESCORE_WITH_SHIFT, use the original_q_num
      // to index into the student's answer sheet
      const lookupKey = String(entry.orig!)
      studentAnswer = (answers[lookupKey] || '').toUpperCase().trim()
      correct = studentAnswer === entry.ans
      if (correct) correctedScore += 1
    }

    questionResults.push({
      correctedQ: entry.q,
      originalQ: entry.orig,
      action: entry.action,
...
export async function GET(req: Request) {
  ...
  if (!sessions || sessions.length === 0) {
    ...
    const { data: allSessions } = await supabase
      .from('exam_sessions')
      .select('id, exam_name, score, created_at')
      .limit(20)
    return NextResponse.json({
      message: 'No 200Q sessions found. All recent exams:',
      allSessions,
    })
  }
  ...
  for (const session of sessions) {
    ...
    const studentName = (session as any).profiles?.full_name || (session as any).profiles?.email?.split('@')[0] || 'Unknown'

    const originalScore = session.score || 0
    const { correctedScore, questionResults } = rescoreSession(answers)
    const correctedPercent = Math.round((correctedScore / 200) * 100 * 10) / 10
    const originalPercent = Math.round((originalScore / 200) * 100 * 10) / 10
    const scoreDelta = correctedScore - originalScore

    // Questions that changed from wrong→right or right→wrong
    const changedQuestions = questionResults.filter(r => {
      if (r.action === 'AWARD_AUTO_CREDIT') return false
      // Only flag questions in the shifted range (Q29+) since Q1-27 are graded same
      return r.correctedQ >= 29
    })
    const newlyCorrect = changedQuestions.filter(r => r.correct && r.originalQ !== r.correctedQ)
    const stillIncorrect = questionResults.filter(r => !r.correct)
    ...
    results.push({
      ...
      correctedPercent,
      scoreDelta,
      hasAnswerSheet: !!sheet,
      answersFound: Object.keys(answers).length,
      autoCredits: 4,
      questionResults,
      incorrectAfterRescore: stillIncorrect.map(r => ({
        q: r.correctedQ,
        system: r.system,
        topic: r.topic,
  ...
  const summary = {
    ...
    sessions: results.map(r => ({
      ...
      examDate: r.examDate?.split('T')[0],
      originalScore: `${r.originalScore}/200 (${r.originalPercent}%)`,
      correctedScore: `${r.correctedScore}/200 (${r.correctedPercent}%)`,
      delta: `${r.scoreDelta >= 0 ? '+' : ''}${r.scoreDelta} points`,
      hasAnswerSheet: r.hasAnswerSheet,
      answersFound: r.answersFound,
    })),
    fullResults: results,
  }
